...
以 `dim=0` 的 gather 为例：取出来的值在 `input` 里位于哪一列，在 `out` 里就依然位于哪一列。也就是说，**gather 只负责根据 `index` 决定"取第几行"，"第几列"完全由元素当前所在的位置决定**。

`index` 形状是 2×2，`out` 也是 2×2，逐个位置看：

- 取 `out[0][0]`：位于第 0 列，`index[0][0]=0` → 取 `input` 第 0 行；列不变 → 取第 0 列 → `input[0][0] = 1`
- 取 `out[0][1]`：位于第 1 列，`index[0][1]=1` → 取 `input` 第 1 行；列不变 → 取第 1 列 → `input[1][1] = 4`
- 取 `out[1][0]`：位于第 0 列，`index[1][0]=2` → 取 `input` 第 2 行；列不变 → 取第 0 列 → `input[2][0] = 5`
- 取 `out[1][1]`：位于第 1 列，`index[1][1]=0` → 取 `input` 第 0 行；列不变 → 取第 1 列 → `input[0][1] = 2`

**一句话**：`dim=0` 时 `index` 只决定"行号"，列号由当前元素所在的位置自然决定，所以列坐标全程不变。这也是为什么 `out[1][1]` 取的是 `input[0][1]` 而不是 `input[0][0]`——行变了，列没变。

### 2. "index 里的值是行下标"是什么意思

`dim=0` 时，`index` 指定的是"去 `input` 的哪一行取数据"。可以把 `index` 当成一个"行号清单"，它明确告诉你输出里每个位置该去 `input` 的哪一行拿数据。

`input` 有 3 行：第 0 行 `[1., 2.]`、第 1 行 `[3., 4.]`、第 2 行 `[5., 6.]`；`index` 里的数字是 `0, 1, 2, 0`，全都是合法行号。

- 取 `out[0][0]`：`index[0][0]=0` → 去第 0 行拿数据；列坐标不变（第 0 列）→ `input[0][0] = 1`
- 取 `out[0][1]`：`index[0][1]=1` → 去第 1 行拿数据；列坐标不变（第 1 列）→ `input[1][1] = 4`
- 取 `out[1][0]`：`index[1][0]=2` → 去第 2 行拿数据；列坐标不变（第 0 列）→ `input[2][0] = 5`
- 取 `out[1][1]`：`index[1][1]=0` → 去第 0 行拿数据；列坐标不变（第 1 列）→ `input[0][1] = 2`

**为什么叫"行下标"**：`dim=0` 指定的维度就是"行"这个维度，`index` 里的数字 `0、1、2` 直接对应 `input` 的第 0、1、2 行，只负责决定行号，不负责决定列号。所以 `index` 里装的不是数据本身，而是"去 `input` 里取第几行"的指示牌。
...
> 这里说的列坐标不变，那出发点是谁，以谁的列坐标？

（按上文口径：出发点 / 参照系是 **`index` 中元素自身所处的位置**，即 `index` 与 `out` 形状一致时，该元素在 `index` 里的列号 `j` 直接用作出参列号。）
